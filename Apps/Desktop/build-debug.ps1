...
Push-Location $repo
try {
    swift build --product OpenPocketCineDesktop --configuration debug
    if ($LASTEXITCODE -ne 0) { throw 'Swift desktop core build failed.' }
    $env:OPC_CORE_LIB_DIR = Join-Path $repo '.build\x86_64-unknown-windows-msvc\debug'
    Push-Location $desktop
    try {
        cargo build --profile debug-camera -p opc-monitor
        if ($LASTEXITCODE -ne 0) { throw 'Rust desktop build failed.' }
        Copy-Item (Join-Path $env:OPC_CORE_LIB_DIR 'OpenPocketCineDesktop.dll') '.\target\debug-camera\OpenPocketCineDesktop.dll' -Force
    } finally { Pop-Location }
} finally { Pop-Location }

Write-Host "Built $desktop\target\debug-camera\opc-monitor.exe"
